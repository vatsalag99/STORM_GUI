...
if exist([exp_folder '/cropped/'])~=7
    ...
    mkdir([exp_folder '/cropped/conv_561/'])
	mkdir([exp_folder '/cropped/conv_561_ds/'])	
...
parfor k = 1:num_images
    ...
    imwrite(A3w, [exp_folder '/cropped/conv_561/' ...
        sprintf('%03d',k) '.tif']);
    A3w_small = imresize(A3w, 0.1);
    imwrite(A3w_small, [exp_folder '/cropped/conv_561_ds/' ...
        sprintf('%03d',k) '.tif']);
end
